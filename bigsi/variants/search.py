...
class BIGSIVariantSearch(object):
    ...
    def search(self, ref, pos, alt="X"):
        var_name = "".join([ref, str(pos), alt])
        variant_probe_set = self._create_variant_probe_set(var_name=var_name)
        return {var_name: self.genotype_alleles(variant_probe_set.refs, variant_probe_set.alts)}

    def _search_for_alleles(self, ref_seqs, alt_seqs):
        results = {"ref": [], "alt": []}
        for ref in ref_seqs:
    ...
    def search(self, gene, ref, pos, alt):
        ...
        for var_name in self.aa2dna.get_variant_names(gene, mut_name, True):
            mut = Mutation(reference=self.reference,
                           ...
                           gene=gene,
                           mut=mut_name)
            variant_probe_set = self._create_variant_probe_set(var_name)
